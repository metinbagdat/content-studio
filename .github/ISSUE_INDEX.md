...
WordPress SEO hub plugins live in **`metinbagdat/wp-seo-hub`**. Canonical long-form blog is **`blog.egitim.today`**. LearnCon `/blog` is product news + 301 after migrate (CS-WP-05). First WP posts **#15/#20** are live. Durable GSC OAuth (CS-WP-03b) waits until posts are **indexed**. **Do not** connect Soro autopilot. Root `lib/` stays until **CS-M2**. LearnCon **LC-G6** stays in `metinbagdat/learncon`.
